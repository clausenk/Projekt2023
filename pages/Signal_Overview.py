...
bubble_plot = alt.Chart((subset_df.loc[(subset_df['Kategorie'] == 'Signal')])).mark_circle().encode(
    x=alt.X('Zeit'),
    y=alt.Y('Certainty'),
...
# Add the text marks
bubble_plot_text = alt.Chart((subset_df.loc[(subset_df['Kategorie'] == 'Signal')])).mark_text(
    align='left',
    baseline='middle',
    dx=14,
    fontSize=18,
